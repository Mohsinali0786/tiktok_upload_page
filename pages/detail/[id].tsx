...
const Detail: NextPage<IProps> = ({ postDetails }) => {
  ...
  const handleLike = async (like: boolean) => {
    ...
    if (userProfile) {
      const { data } = await axios.post(`${API_BASE_URL}/${VIDEOS_ROUTE}${post?.id}/heart`, {
      ...
      setPost({ ...post, hearts: post.hearts + 1 });
    }
  };
  ...
  const addComment = async (e: React.FormEvent) => {
    ...
    if (userProfile && comment) {
      ...
      const { data } = await axios.put(`${API_BASE_URL}/${VIDEOS_ROUTE}${post.id}`, {
        userId: userProfile.id,
        comment,
      }, {
        headers: {
    ...
    <div className="flex w-full absolute left-0 top-0 bg-white flex-wrap lg:flex-nowrap">
      ...
      <div className="relative w-[1000px] md:w-[900px] lg:w-[700px]">
        <div className="lg:mt-20 mt-10">
          ...
          <p className="px-10 text-lg text-gray-600">{post.title}</p>
...
export const getServerSideProps = async ({
  ...
  const { data } = await axios.get(`${API_BASE_URL}/api/post/${id}`);
